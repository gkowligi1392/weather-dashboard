var cityList = [];
var id = "2ee98bc525fe0c9cf53af3480ed5e2ff";
...
function createCityList(){
    // Clear cityList before adding new city
    $(".cityList").empty();
    ...
    $.ajax({
        ...
        for (i = 0; i < data.list.length; i++) {
            if (data.list[i].dt_txt.search("15:00:00") != -1) {
                ...
                $(".forecast").append(
                    `<div class="card bg-info shadow m-4">
                        <div class="card-body">
...
function getUVI(id, cityLat, cityLong) {
    ...
    $.ajax({
        ...
        if (data.value >= 8) {
            ...
            $(".cityToday").append(`<p>UV Index: <span class="badge badge-warning p-2">${data.value}</span></p>`);
        }
        else {
            $(".cityToday").append(`<p>UV Index: <span class="badge badge-success p-2">${data.value}</span></p>`);
        }
    })
}
